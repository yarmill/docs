...
[SPORT] **IBU ID** (biathlon instances): the athlete's identifier in the IBU database, required to connect the athlete to their IBU race results/stats (SIWIDATA — see 5.11, A.3). Observed live (Biathlon Canada, 2026-07-15): configured as a field **inside the Personal information section** (in-place pencil edit) — i.e. per-instance config can add fields to Personal information itself, not only as separate sections. Where users find it: the ID is the **last segment of the athlete's profile URL on biathlonworld.com** (site menu **BIATHLETES** → athlete profile), e.g. `https://www.biathlonworld.com/athlete/davidova-marketa/BTCZE20301199701` → ID `BTCZE20301199701`. Format observed: `BT` + 3-letter NOC code + digits.
