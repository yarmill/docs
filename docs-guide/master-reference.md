...
EN UI label: **Goals** (distinct from "Goals (Motivy)", the 4-week plan view in 5.1). In the main menu (under **Planning** in the new left-side UI; **Other** in the older top-menu UI). **Web only** (incl. PWA). Athletes manage their own goals and **see only their own** (no group selector, athlete list, or group overview); coaches choose a group → athlete to manage their athletes' goals, **and can create goals for themselves**. Goals are bucketed by season (**Current season** / **Past season** / **Other**). Strong fit for pre-season and post-season conversations and retrospectives (valued by mental coaches).

- **Goal anatomy:** title, description, **state**, **season**, **priority** (Low / Medium / High, shown as `!`/`!!`/`!!!`), **supervisors** (the coaches following the goal), **category** (two-level: top category → subcategory), **key results**, **final evaluations**, and an **Activity** log. Create / verify / delete sit in a floating **tool bar at the bottom** of the goal panel (**New goal**, shortcut **N** · **Verification** · trash), not at the top right.
...
- **Final evaluations** — closing written assessments; athlete and/or coach can each add one. **Gated on a final state**: a final evaluation cannot be added until the goal is set to **Completed**, **Canceled** or **Failed** (confirmed by product owner 09/2026). Distinct from **Activity** comments (plain, timestamped text, addable at any time) and the automatic change-log.
- **Verification** — a panel (checklist icon in the **tool bar at the bottom** of the goal, with a dot badge: red = a required item missing, amber = only recommended ones) that marks each field **required (red)** or **recommended (amber)**, with **green checks** for completed ones (title, description, supervisor, category, key results), and links to **Open guidelines** — the team's own guidelines document (an external, per-team configured URL, e.g. a Notion page). Completeness thresholds (e.g. a minimum number of key results) are instance-specific [CONFIG]. **Not a Yollanda surface** — the panel carries no Yollanda branding (verified 09/2026; earlier notes calling it "Document check (Yollanda persona)" were wrong).
...
Centralized clinical records: injuries, illnesses, treatments, rehab status, return-to-training/-performance, medical history, clearances. Verified new UI (06/2026): module label **Injuries & Illnesses**; group overview columns **Status / Athlete / Open health problems / Expected return to full training** (overdue returns flagged, e.g. "61 days overdue"); records split **Open / Closed**. Per record: title + type (injury/illness icons), **Properties** (Open/closed; training limitation e.g. **No training**; **Set Responsible Staff**), **Key dates** (start; expected return), **Note**, **Diagnosis (OSIICS standard, **version 16**; linked code list; labels in Czech as well as English, the code unchanged)**, **Files**, and an **Activity** log with timestamped comments; **New record** in the tool bar (keyboard shortcut `N`; was "New quick entry" until 09/2026) opening a short dialog. A record's training-limitation flag (full / partial / none) feeds **Team Daily Readiness**; closing a record sets it to **Full training** automatically (verified in a capture, 09/2026). Access is permission-gated. Purpose: not just "who's available" but injury-pattern analysis (recurring injuries, by age) for prevention.

Re-verified live 09/2026 (AFC Richmond, coach + athlete sessions): a new record defaults to Classification **New**, Treatment Status **Open**, training limitation **Modified training**, dated today, and auto-saves from there — **quick entry is the only part that waits for a Save**. **Classification** is New | Recurring; the **Circumstances** categories (Activity · Injury mechanism · Location · Severity · Surface) are Yarmill codelists configured per instance, *not* OSIICS. The **diagnosis** flow is search → pick the code → pick a **Side** → **Add** (`Ctrl`+`↵`); **an illness record has no Side**, and the added diagnosis carries auto-tags for body region and structure. An instance can be configured with a list of **pinned diagnoses** the team chooses, which sit above the full codelist [CONFIG] — a pin list, not a usage-ranked one (09/2026). The **closure date** is editable, and **a closed record can be reopened** by setting its treatment status back to Open. The status pill on the group overview is the athlete's **worst** current training limitation across their open records. **No reporting is built on diagnosis codes yet** (09/2026) — the value of coding today is a consistent, portable record.
...
[SPORT] **IBU ID** (biathlon instances): the athlete's identifier in the IBU database, required to connect the athlete to their IBU race results/stats (SIWIDATA — see 5.11, A.3). Observed live (Biathlon Canada, 2026-07-15): configured as a field **inside the Personal information section** (in-place pencil edit) — i.e. per-instance config can add fields to Personal information itself, not only as separate sections. Where users find it: the ID is the **last segment of the athlete's profile URL on biathlonworld.com** (site menu **BIATHLETES** → the **Search athlete** field, results filter as you type; nationality and gender filters also available — live-verified in-browser 2026-07-15), e.g. `https://www.biathlonworld.com/athlete/davidova-marketa/BTCZE20301199701` → ID `BTCZE20301199701`. Format observed: `BT` + 3-letter NOC code + digits. The Yarmill field is **case-insensitive** (chat 2026-07-15 — lowercase IDs from URLs work). Any role can fill it in — administrator, coach, or the athlete on their own card (confirmed in chat 2026-07-15). External IDs incl. `ibu-id` can also be bulk-set via the user-import workbook in the admin portal (admin.yarmill.com/UserImport) — but that portal is **Yarmill-internal only** (no client or client-admin access; confirmed in chat 2026-07-15), so never document it as a user-facing path.
...
[CORE] **Ingest API.** Yarmill exposes an HTTPS JSON API for accepting data from third parties (documented at `/en/api-reference/*`; OpenAPI spec live at https://ingest.ms.yarmill.com/swagger/index.html — that spec is the authority). Base URL `https://ingest.ms.yarmill.com`; endpoint paths are kept stable, but partners are told to treat the base URL as configuration.

- **Auth** — an **AppToken** in the `Authorization` header (`Authorization: AppToken <token>`; the scheme word is `AppToken`, not `Bearer`). **Issued manually by Yarmill support on request — there is no self-service token endpoint** (confirmed 08/2026; partners email support@yarmill.com and should ask for a **test token** and a **production token** together, and come back to support for renewal). Integration briefs that describe tokens as "provided via the Yarmill Auth API" are misleading on this point. Long-lived but time-limited; the source side may revoke at any time, Yarmill only on a security incident — so partners must keep it in config/secret storage, not in code. **All three endpoints require it, including `/api/ping`** (verified 08/2026 against the live service).
- **Endpoints** (all `POST`): `/api/data/upload` (the main one), `/api/data/files` (registers metadata for a file already uploaded to the ms-filestorage service — ingest never receives the file stream), `/api/ping` (connectivity + token check; needs an explicit `Content-Length: 0` or IIS rejects it with `411` before it reaches the API; a bad token returns `401` with the plain-text body `Invalid token`, whereas the two data endpoints return `401` with an empty body).
- **Upload payload** — `originType` (the data type, usually the source entity's name; the same type must always carry the same structure in `data`) and `data` (any valid JSON object or array) are the substantive fields; `originId` (the source's own id), `operationType` (`C`/`U`/`D`), `metadata` (key-value, **string values only**), and `tags` (string array) are optional. **`originId` + `originType` together form the identifying key** — `originId` alone is not unique. Without `originId`, re-sending a record duplicates it instead of updating it. ISO 8601 (`YYYY-MM-DDTHH:mm:ss.sssZ`) is the expected date format.
- **Behaviour** — accepts Create/Update/Delete on any uniquely identified record; accepts **historical data** with no cut-off; always confirms acceptance or rejection; **does not validate data quality** (a `200` means received, not correct). **Responses carry no body at all — the status code is the entire response** (confirmed 08/2026): no created-record id, no echo of the payload, no structured error object. Consequence for integrators: the partner's own `originId` is the only link between a record in their system and the same record in Yarmill, since Yarmill never hands back an id. Codes: `200` / `400` (invalid request) / `401` (missing or invalid token) / `403` (valid token, not permitted for the operation) / `500` (retryable). Where a body does appear it is plain text and not a contract — `/api/ping` answers a bad token with `Invalid token`, while the two data endpoints return an empty `401`.
- **Partner guidance** we give integrators: own delivery (queue events, only mark done on success); one record per request at a fixed granularity (e.g. athlete × measurement × type × operation) sent near-real-time rather than in nightly batches; retry `500`/network errors with a growing gap, never retry `400`/`403`; agree any change to an existing `originType`'s structure with Yarmill **before** sending it.
- **Open gaps** (flagged as `TODO(yarmill)` in the docs): the ms-filestorage upload endpoint, auth, and limits for step 1 of the file flow; and the `ReferenceObjectType` (`sourceType`) value→object mapping, exposed in the spec as bare integers `1,10,20,…,90`. Also undocumented anywhere: rate limits, and whether `operationType: "D"` is a hard or soft delete. The spec also contradicts itself on required fields — the `/api/data/upload` description marks `originType` and `data` required while `DataRequestModel` declares no required properties and marks all nullable.
