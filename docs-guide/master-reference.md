...
- **Auth** — an **AppToken** in the `Authorization` header (`Authorization: AppToken <token>`; the scheme word is `AppToken`, not `Bearer`). Issued by Yarmill; a **test token** is available for integration work. Long-lived but time-limited; the source side may revoke at any time, Yarmill only on a security incident — so partners must keep it in config/secret storage, not in code. **All three endpoints require it, including `/api/ping`** (verified 08/2026 against the live service).
...
- **Open gaps** (flagged as `TODO(yarmill)` in the docs): success-response body shape is undocumented (spec says `200` with no schema); how a partner actually obtains/renews a token (briefs say "via the Yarmill Auth API", which isn't in the Ingest spec); the ms-filestorage upload endpoint, auth, and limits for step 1 of the file flow; and the `ReferenceObjectType` (`sourceType`) value→object mapping, exposed in the spec as bare integers `1,10,20,…,90`. The spec also contradicts itself on required fields — the `/api/data/upload` description marks `originType` and `data` required while `DataRequestModel` declares no required properties and marks all nullable.
