...
- **Behaviour** — accepts Create/Update/Delete on any uniquely identified record; accepts **historical data** with no cut-off; always confirms acceptance or rejection; **does not validate data quality** (a `200` means received, not correct). Errors: `200` / `400` (invalid request + message) / `401` (missing or invalid token) / `403` (valid token, not permitted for the operation, + message) / `500` (retryable).
